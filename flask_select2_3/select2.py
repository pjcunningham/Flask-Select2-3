...
class Select2(object):
    ...
    def add_loader(self, loader):
        if not loader.name in self.loaders:
            ...
            raise LoaderError(message=f'Call to add_loader with name : "{loader.name}" already exists')

    def get_loader(self, name):
        _loader = self.loaders.get(name)
        if not _loader:
            raise LoaderError(message=f'Call to get_loader with name : "{name}" not found')
        return _loader
